...
public class TestNGCucumberRunner {
	...
	public TestNGCucumberRunner(ITestContext context, Class<?> clazz) {
		...
		if (allParam.containsKey("features")) {
			List<String> featurPaths = Arrays.asList(allParam.getOrDefault("features", "").split(","));
			runtimeOptions.getFeaturePaths().addAll(featurPaths);
		}

		if (allParam.containsKey("glue")) {
			List<String> glues = Arrays.asList(allParam.getOrDefault("glue", "").split(","));
			runtimeOptions.getGlue().addAll(glues);
		}

		if (allParam.containsKey("tags")) {
			runtimeOptions.getFilters().add(context.getCurrentXmlTest().getParameter("tags"));
		}
